...
function Constraint(properties)
{
	...
	//this.yoValue = properties.yoValue;
}
...
function generateTestCases()
{
	...
	for ( var funcName in functionConstraints )
	{
		...
		//var yoparams = initalizeParams(functionConstraints[funcName])
		//console.log( params );
		...
		{
			a[constraints[i].ident]=1+(a[constraints[i].ident] || 0);
		...
		for(var key in a)
		{
			allargs.push(params[key]);
		}
		 allargs = product(allargs);
		...
		if (node.type === 'FunctionDeclaration') 
		{
			...
			//	}
				...
				if( child.type == "BlockStatement" && child.body[0].type == "VariableDeclaration" && child.body[0].declarations[0].type == "VariableDeclarator")
					{
						if(child.body[0].declarations[0].init.type == "CallExpression" && child.body[0].declarations[0].init.callee.name == "format")
						{
							if( child.body[0].declarations[0].init.arguments[0].name == "phoneNumber")
								{
									for( var p =0; p < params.length; p++ )
									{
								...
								// A fake path to a file
												...
												altValue: '\"'+child.body[2].test.right.value+'0\"',
												funcName: funcName,
												kind: "phoneNumber",
												operator : child.operator,
												expression: expression
				...
				if( child.type == "AssignmentExpression" && child.left.name == "phoneNumber" )
					for( var p =0; p < params.length; p++ )
					{
					...
					{
						if(child.left.operator == "!" && child.left.argument.name == "options" )
						{		
							for( var p =0; p < params.length; p++ )
							{
				...
				if( child.type == "AssignmentExpression" && child.left.name == "formatString" )
					{
						for( var p =0; p < params.length; p++ )
						{
							if( child.left.name == params[p] )
					...
					if( child.type == "MemberExpression" && child.object.name == "phoneNumber" )
					{
						for( var p =0; p < params.length; p++ )
						{
